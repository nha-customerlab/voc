// lib/data.ts — ชั้นข้อมูล (ตอนนี้เป็น mock ในหน่วยความจำ)
// สลับเป็น Supabase: เปลี่ยน body ของฟังก์ชันด้านล่างให้เรียก supabase (ดู lib/voc.ts ในโฟลเดอร์ nextjs-starter)
...
const VOICES: { topic: string; voice: string; sent: Sentiment; cat: string; owner: string }[] = [
  { topic: 'แจ้งซ่อมระบบประปา', voice: 'น้ำประปาในห้องไหลอ่อนมาก บางวันไม่ไหลเลย รบกวนส่งช่างมาตรวจสอบ', sent: 'Negative', cat: 'คุณภาพโครงการและการก่อสร้าง', owner: 'ฝ่ายปรับปรุงและบำรุงรักษาชุมชน' },
  { topic: 'สอบถามเงื่อนไขเช่าซื้อ', voice: 'อยากทราบเงื่อนไขการเช่าซื้อ ต้องวางเงินดาวน์เท่าไหร่ ผ่อนกี่ปี', sent: 'Neutral', cat: 'การซื้อ/เช่าซื้อ', owner: 'ฝ่ายการตลาด' },
  { topic: 'ร้องเรียนความสะอาดส่วนกลาง', voice: 'พื้นที่ส่วนกลางสกปรกมาก ขยะไม่ได้เก็บหลายวัน ช่วยดูแลด้วย', sent: 'Negative', cat: 'คุณภาพโครงการและการก่อสร้าง', owner: 'ฝ่ายสื่อสารองค์กร' },
  { topic: 'ชื่นชมเจ้าหน้าที่สาขา', voice: 'เจ้าหน้าที่สาขาบริการดีมาก ให้คำแนะนำชัดเจนและสุภาพ ประทับใจมาก', sent: 'Positive', cat: 'การให้ข้อมูลโครงการ', owner: 'ฝ่ายการตลาด' },
  { topic: 'ขอผ่อนผันค่าเช่า', voice: 'เดือนนี้รายได้ลดลง ขอผ่อนผันการชำระค่าเช่าออกไปก่อนได้ไหม', sent: 'Neutral', cat: 'การผ่อนชำระที่อยู่อาศัย', owner: 'ฝ่ายบริหารสินเชื่อและหนี้' },
  { topic: 'ระบบจองออนไลน์ขัดข้อง', voice: 'จองคิวผ่านเว็บไซต์ไม่ได้ ระบบค้าง กดยืนยันแล้วเด้งออก', sent: 'Negative', cat: 'ระบบจองบ้านออนไลน์', owner: 'ฝ่ายเทคโนโลยีสารสนเทศ' },
  { topic: 'สอบถามโอนกรรมสิทธิ์', voice: 'ผ่อนครบแล้ว ต้องเตรียมเอกสารอะไรบ้างสำหรับการโอนกรรมสิทธิ์', sent: 'Neutral', cat: 'การซื้อ/เช่าซื้อ', owner: 'ฝ่ายบริหารงานขาย' },
  { topic: 'ไฟส่องสว่างชำรุด', voice: 'ไฟทางเดินและลานจอดรถดับหลายจุด กลางคืนมืดและไม่ปลอดภัย ช่วยซ่อมด่วน', sent: 'Negative', cat: 'คุณภาพโครงการและการก่อสร้าง', owner: 'ฝ่ายปรับปรุงและบำรุงรักษาชุมชน' },
];

function pick<T>(a: T[], i: number) { return a[i % a.length]; }

// สร้างชุดข้อมูลจำลอง 60 รายการ (คงที่ เพื่อให้ SSR/หน้าเว็บสอดคล้อง)
const DATA: Voc[] = Array.from({ length: 60 }, (_, i) => {
...
export async function pipelineStats(): Promise<Record<string, number>> {
  ...
  CASE_STATUS.forEach(s => (st[s] = 0));
  DATA.forEach(x => (st[x.status] = (st[x.status] || 0) + 1));
  return st;
}
export async function sentimentStats() {
  const t = DATA.length;
  const c = { Positive: 0, Neutral: 0, Negative: 0 };
  DATA.forEach(x => c[x.sentiment]++);
  return { total: t, ...c, posPct: Math.round(c.Positive / t * 100), negPct: Math.round(c.Negative / t * 100), neuPct: Math.round(c.Neutral / t * 100) };
}
export async function channelStats() {
  return CHANNELS.map(name => {
    const rows = DATA.filter(x => x.channel === name);
